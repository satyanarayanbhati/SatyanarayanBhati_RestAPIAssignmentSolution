package com.greatlearning.employee.management.config;
...
public class SwaggerConfig {
	...
	public Docket EmployeeApi() {
		...
		return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo()).groupName("Employee-Management-API").select()
				.apis(RequestHandlerSelectors.basePackage("com.greatlearning.employee.management")).build()
				...
				.globalResponseMessage(RequestMethod.GET, Arrays.asList(new ResponseMessageBuilder().code(500).message(
						...
						new ResponseMessageBuilder().code(403)
								...
								.build(),
						new ResponseMessageBuilder().code(401).message("you are not authorized").build(),
				...
				.globalResponseMessage(RequestMethod.DELETE, Arrays.asList(
						...
						new ResponseMessageBuilder().code(403)
								...
								.build(),
						new ResponseMessageBuilder().code(401).message("You are not authorised").build(),
						new ResponseMessageBuilder().code(400)
								.message("Bad Request: Check the API URL or the request parameters again").build(),
	...
	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("Employee Management API").description("Employee Management API referance for developers")
				.termsOfServiceUrl("http://fakeEmployeeManagement.com")
				.contact(new Contact("Employee Management API", "http://fakeEmployeeManagement.com", "fakeEmployeeManagement@gmail.com"))
				.license("EMployee management License").licenseUrl("http://fakeEmployeeManagement.com").version("1.0").build();
	}
